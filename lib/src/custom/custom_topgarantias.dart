...
class ReportTop5Antiguas {
  ...
  static Future<pw.Document> build() async {
    ...
    final items = await Future.wait(top5.map((e) async {
      ...
      final prodData = prodQuery.docs.isNotEmpty ? prodQuery.docs.first.data() : null;
      ...
      final diasTranscurridos = fechaVencimiento == null
          ? 'N/D'
          : DateTime.now().difference(fechaVencimiento).inDays.abs().toString();
